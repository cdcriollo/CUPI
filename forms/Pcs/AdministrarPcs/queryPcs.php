...
$count=0;
$result=mysql_query("SELECT COUNT(*) AS count FROM pcs ".$filters,$conexion);
if($result==true)
{
...
$SQL = "SELECT * FROM  pcs $filters ORDER BY $sidx $sord LIMIT $start,$limit"; 
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
    $s .= "<row id='". $row['Nopc']."'>";            
    $s .= "<cell>". $row['Nopc']."</cell>";
	$s .= "<cell><![CDATA[". $row['numSala']."]]></cell>";
	$s .= "<cell><![CDATA[". $row['estado']."]]></cell>";
   	$s .= "<cell><![CDATA[". $row['estadoocupacion']."]]></cell>";
	$s .= "</row>";
}
$s .= "</rows>"; 
