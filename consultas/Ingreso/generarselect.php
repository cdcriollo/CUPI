...
function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
{
 ...
 $query_JRSelects = "SELECT * from pcs WHERE numSala = ".$_GET['id'];
 ...
 if ($totalRows_JRSelects > 0){
	 echo '<option value="'.$row_JRSelects['Nopc'].'">'.$row_JRSelects['Nopc'].'</option>';
	while ($fila = mysql_fetch_array($JRSelects)) {
        echo '<option value="'.$fila['Nopc'].'">'.$fila['Nopc'].'</option>';
    };
