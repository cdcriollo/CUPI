...
function ObtenerHorario($idhorario,$database_conexion, $conexion,$reserva)
{
	// Realizo consulta sobre la tabla horario
    mysql_select_db($database_conexion, $conexion);
	$query_JRHorario = "SELECT codDia, horainicio, horafinal FROM horario WHERE idHorario= $idhorario and No_reserva= '$reserva'";
...
mysql_select_db($database_conexion, $conexion);
$query_JRCmatriculaA = "select u.nombreUsu, u.apellidos, m.codUsuario,m.pc,p.numSala,m.idHorario from matricula m  inner join usuarios u on (m.codUsuario=u.codUsuario)  inner join pcs p on (m.pc=p.Nopc)  where m.codAsignatura= '$codigo' and m.grupo=$grupo and m.No_reserva= '$reserva' and m.Estado='Activa' order by pc,idHorario";
...
<table class="tableUI" width="600" style="margin-left:10px; margin-top:15px;"  border="1" id="addstudents">
  ...
  <?php do { ?>
	 ...
	 <tr>
      ...
      <?php  $horario= ObtenerHorario($idhorario,$database_conexion, $conexion,$reserva); ?>
