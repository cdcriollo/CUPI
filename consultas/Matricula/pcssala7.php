...
if(isset($_POST['sala'],$_POST['asignatura'],$_POST['grupo'],$_POST['opcion'],$_POST['reserva']))
{
	...
	$stylefree='style="color:#009900; font-weight:bold; font-size:14px;"';
	 ...
	 function verificarestadopc ($database_conexion, $conexion, $pc, $asignatura, $grupo, $reserva)
	 {
		  mysql_select_db($database_conexion, $conexion);
		  $query_JRPcmatriculasala7 = "SELECT pc FROM matricula WHERE pc = $pc and  Estado='Activa' and codAsignatura= '$asignatura' and grupo= $grupo and No_reserva = '$reserva'";
	...
	<?php
	   ...
	   if ($totalRows_JRPcsala7 > 0 && $sala!=0) 
	   {
	  ...
	  <?php
		 ...
		 do
		 {
		   ...
		   $estadopc=verificarestadopc($database_conexion, $conexion, $pcsala7, $asignatura, $grupo, $reserva);
		   
		   if($estadopc==1)
		   {?>
			 
			  <?php if ($opcion==2){?>
			  <td  <?php  echo $styleocupado ?>>
			  <input type="checkbox" value="<?php echo $row_JRPcsala7['Nopc']; ?>" class="freepc"/>  <?php echo $pcsala7 ?> </td> 
			  <?php } else if($opcion==1){?> <td  <?php  echo $styleocupado ?>><input type="checkbox" value="<?php echo $row_JRPcsala7['Nopc']; ?>" class="freepcuser"/>  <?php echo $pcsala7 ?> </td>  <?php } ?>
		   ...
		   {?>
			  <?php if ($opcion==2){?>
				<td <?php echo $stylefree ?>><input type="checkbox" value="<?php echo $row_JRPcsala7['Nopc']; ?>" class="freepc"/><?php echo $pcsala7 ?></td>       <?php }  else if($opcion==1){?> <td  <?php  echo $stylefree ?>><input type="checkbox" value="<?php echo $row_JRPcsala7['Nopc']; ?>" class=  "freepcuser"/>  <?php echo $pcsala7 ?> </td>  <?php } ?>
