...
$query_JRidhorario = "SELECT idHorario FROM horario WHERE codAsignatura = '$asignatura' and codGrupo=$grupo AND sala=$sala AND estadohorario='activo' and No_reserva= '$reserva' order by idHorario";
	...
	mysql_free_result($JRPcsMatriculados);
	 ...
	 {  
		...
		$totalRows_JRPcGenerado = mysql_num_rows($JRPcGenerado);
		 ...
		 {
		    $computador= $row_JRPcGenerado['Nopc'];
			$response=array("error"=>0,"pc"=>$computador);
		    echo json_encode($response);	
