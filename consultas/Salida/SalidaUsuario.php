...
if($totalRows_JRUsuarios > 0){
	...
	mysql_select_db($database_conexion, $conexion);
	$query_JRSalidas = "select i.actividad, i.sala, i.computador, i.estado, i.codIngreso, u.nombreUsu, i.codAsignatura, i.codGrupo from ingreso_salida i inner join usuarios u on (i.codUsuario=u.codUsuario)  where i.fecha='$fecha' and u.codUsuario=$usuario and i.estado <> 1 order by i.codIngreso desc limit 1";
	mysql_query("SET NAMES 'utf8'");
   ...
   if ($totalRows_JRSalidas > 0){
	...
	$arraysalidas[8]=$row_JRNombreAsignatura['nomAsignatura'];
