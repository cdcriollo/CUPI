...
(function($){
  $.validation = function(params) {
	...
	var defaults = {
		...
		//contentmsg:"None",
		fields:[
			{id:"None",validations:{clength:["0-0","None"],LaN:[false,"None"],email:[false,"None"],required:[false,"None"], number:[false,"None"]}}				  
		]
		,
  ...
  //  alert(opciones.fields[0].id + "-" +opciones.fields[1].id+"-"+opciones.fields[2].id+"-"+ opciones.fields[3].id);
	...
	//	alert(required);
		...
		{			
			...
			{
				//PATRON:/^[a-z]([0-9a-z_])+$/i
			  // alert(arrayLaN[0]+"-"+arrayLaN[1]);
			//  /[A-Za-zñÑ\s]/
			  msgLaN=arrayLaN[1];
			  resultLaN=checkRegexp($("#"+idfield),/^[a-zñÑ]([0-9a-zñÑ_])+$/i,styleerror,styledone,$("#"+ctmsg),msgLaN);
		...
		if(resultrequired == true && resultlength == true && resultLaN == true && resultemail == true && resultnumber == true)
		{
			arrayValid[i]=true;
